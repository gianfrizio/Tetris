...
window.fitCanvasToViewport = function fitCanvasToViewport() {
    logger.log('ui', 'Fitting canvas to viewport');
    ...
    logger.log('ui', 'Canvas fitted', { isMobile, viewportWidth, viewportHeight });
}
...
let lastTapTime = 0;
// Import configuration values from GameConfig
const TAP_DEBOUNCE = GameConfig.TAP_DEBOUNCE;
const TAP_HOLD_TIME = GameConfig.TAP_HOLD_TIME;
const LONG_PRESS_TIME = GameConfig.LONG_PRESS_TIME;
...
const TAP_BURST_WINDOW = GameConfig.TAP_BURST_WINDOW;
const TAP_BURST_THRESHOLD = GameConfig.TAP_BURST_THRESHOLD;
let lastLongPressTime = 0;       // timestamp of last successful long-press
const LONG_PRESS_COOLDOWN = GameConfig.LONG_PRESS_COOLDOWN;
...
const MOVE_THROTTLE_MS = GameConfig.MOVE_THROTTLE_MS;
...
window.addTouchControls = function addTouchControls() {
    const canvas = document.getElementById('canvas');
    if (!canvas) return;
    ...
    canvas.addEventListener('touchstart', function(e) {
        ...
        longPressTimer = setTimeout(() => {
            ...
            if (now - lastLongPressTime < LONG_PRESS_COOLDOWN) {
                logger.log('touch', 'Ignoring long press due to cooldown');
                return;
            }

            // If quick tap burst detected, don't allow accidental long-presses
            if (recentTapCount >= TAP_BURST_THRESHOLD) {
                logger.log('touch', 'Suppressing long press due to recent tap burst: ' + recentTapCount);
                return;
            }
            ...
            if (!touchMoved && longPressTimer !== null && canvasTouchActive && actualTouchDuration >= LONG_PRESS_TIME) {
                ...
                logger.log('touch', 'Long press activated - conditions met (duration: ' + actualTouchDuration + 'ms)');
                showMobilePauseMenu();
            } else {
                logger.log('touch', 'Long press cancelled - movement detected, too short, or timer cleared', { touchMoved, actualTouchDuration });
            }
        }, LONG_PRESS_TIME);
    }, { passive: false });
        ...
        if (deltaX > 20 || deltaY > 20) { // Increased threshold to be even less sensitive
            if (!longPressTriggered && !touchMoved) {
                ...
                if (longPressTimer) {
                    clearTimeout(longPressTimer);
                    longPressTimer = null;
                    logger.log('touch', 'Long press cancelled due to movement', { deltaX, deltaY });
                }
            }
        }
    }, { passive: false });
    ...
    canvas.addEventListener('touchend', function(e) {
        ...
        if (longPressTriggered) {
            logger.log('touch', 'Long press was triggered, keeping it active');
        ...
        if (longPressTimer) {
            ...
            logger.log('touch', 'Long press timer cancelled - touch too short: ' + touchDuration + 'ms');
        }
        ...
        if (Math.abs(touchEndX - touchStartX) < 20 && Math.abs(touchEndY - touchStartY) < 20 && (touchEndTime - touchStartTime) < TAP_HOLD_TIME) {
            ...
            logger.log('touch', 'Registered quick tap, burst count: ' + recentTapCount);
        }

        if (!gameState.get('gameStartRequested')) {
            canvasTouchActive = false;
            touchMoved = false; // Reset for next touch
            return;
        }
        ...
        if (Math.abs(deltaX) < minSwipeDistance && Math.abs(deltaY) < minSwipeDistance && deltaTime < TAP_HOLD_TIME) {
            ...
            if (now - lastTapTime > TAP_DEBOUNCE) {
                ...
                logger.log('touch', 'Tap rotate executed');
            } else {
                logger.log('touch', 'Tap ignored due to debounce');
            }
            canvasTouchActive = false;
            touchMoved = false; // Reset for next touch
            return;
...
window.simulateKeyPress = function simulateKeyPress(key, options = {}) {
    logger.log('keyboard', 'Simulating key press: ' + key, options);
    ...
    const now = Date.now();
    const SIMULATE_KEY_COOLDOWN_MS = GameConfig.SIMULATE_KEY_COOLDOWN_MS;
    if (now - last < SIMULATE_KEY_COOLDOWN_MS) {
        logger.log('keyboard', 'Skipping simulated key (cooldown): ' + key);
        return;
    }
    ...
    try {
        ...
        const KEY_HOLD_MS = GameConfig.KEY_HOLD_MS;
        setTimeout(() => {
                ...
                try {
                    ...
                    setTimeout(() => {
                        if (document.body.contains(currentFocus)) {
                            ...
                            logger.log('keyboard', 'Focus restored to: ' + currentFocus.tagName + ' ' + currentFocus.id);
                        }
                    }, 50);
                } catch(e) {
                    logger.error('keyboard', 'Could not restore focus', e);
                }
            }
        }, KEY_HOLD_MS);
    } catch (e) {
        logger.error('keyboard', 'simulateKeyPress error', e);
    }
}
...
setTimeout(() => {
    logger.log('touch', 'Adding touch controls immediately');
    addTouchControls();
}, 1000);

// Fallback: aggiungi controlli touch anche al body per essere sicuri
function addBodyTouchControls() {
    logger.log('touch', 'Adding fallback touch controls to body');
    ...
    document.body.addEventListener('touchend', function(e) {
        ...
        if (canvasTouchActive) return;
        if (!gameState.get('gameStartRequested')) return;
...
// fitCanvasToViewport, addTouchControls, simulateKeyPress are defined as window.* above
